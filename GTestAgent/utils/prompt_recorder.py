...
class PromptRecorder:
    ...
    def record(cls, prompt, mode):
        ...
        with open(os.path.join(agent_config.agent_output_dir, 'prompts',
                               f'prompt_{cls.state_tag}_{datetime.now().strftime("%H%M%S")}_{mode}.txt'), 'w',
                  encoding='utf-8') as f:
            f.write(prompt_str)
    ...
    def record_gptdroid_conversation(cls, current_prompt, full_prompt):
        ...
        with open(os.path.join(agent_config.agent_output_dir, 'conversation.txt'), 'w') as f:
            f.write(stringify_prompt(zip_messages(full_prompt['system_message'], full_prompt['user_messages'],
                                                  full_prompt['assistant_messages'])))
