...
def get_next_assistant_message(system_message, user_messages, assistant_messages=[], functions=[], model="gpt-3.5-turbo-16k-0613", max_tokens=MAX_TOKENS, function_call_option=None):
    ...
    for _ in range(MAX_RETRY):
        ...
        #     time.sleep(3)
            ...
            logger.error(f'OpenAI API request timed out: {e}. Retrying...')
            time.sleep(3)
            continue
        except openai.APIConnectionError as e:
            logger.error(f'OpenAI API connection error: {e}. Retrying...')
            time.sleep(3)
            continue
        except openai.InternalServerError as e:
            logger.error(f'OpenAI API internal server error: {e}. Retrying...')
            time.sleep(3)
            continue
        except openai.RateLimitError as e:
            logger.error(f'OpenAI API rate limit exceeded: {e}. Retrying...')
            time.sleep(3)
            continue
