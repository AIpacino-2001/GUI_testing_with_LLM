...
class PersistentStorage:
    ...
    def stringify_entries(self, raw_entries, mode='widget_knowledge', show_timestamp=True, show_type=True,
                          max_len=None):
        ...
        for memory_id, metadata, doc in zip(raw_entries['ids'], raw_entries['metadatas'], raw_entries['documents']):
            if mode == 'task_history':
                if len(doc) == 0:
                    continue
                entries.append(
                    self._stringify_entry(memory_id, metadata, doc, show_timestamp=show_timestamp, show_type=show_type))
...
class Memory:
    ...
    def save_snapshot(self, output_dir):
        ...
        with open(os.path.join(output_dir, 'scratch.json'), 'w', encoding='utf-8') as f:
            json.dump(working_memory_record, f, indent=2)

        task_history_record = self.history.stringify_all_entries(mode='task_history')
        with open(os.path.join(output_dir, 'long_term_memory.txt'), 'w', encoding='utf-8') as f:
    ...
    def collect_knowledge(self):
        ...
        for memory_id, metadata, state in zip(widget_knowledge_entries['ids'], widget_knowledge_entries['metadatas'],
                                              widget_knowledge_entries['documents']):
            if len(metadata['observation']) == 0:
                continue

            action_type = metadata['action']
            widget_knowledge[metadata['page']][metadata['widget']].append(
                (int(memory_id), (action_type, metadata['observation'])))

        for memory_id, metadata, state in zip(task_knowledge_entries['ids'], task_knowledge_entries['metadatas'],
                                              task_knowledge_entries['documents']):
